...
<body>
    ...
    <header class="">
        <section class="border-bottom-separator mb-2 " style="height: 180px;">
            ...
            <div class="float-right pt-0 position-relative" style="width: 25%;height: 150px;">
                <div class="position-absolute" style="width: 100%; height: 70px; right: 0; top: -35px">
                    <p class="my-0">&nbsp;</p>
                    <p class="text-right my-0"><strong>
                            Factura Nº <?= $code . ' / ' . date('Y') ?> </strong></p>
                    <p class="text-right my-0 mb-2"><strong>Original</strong></p>
                    <br>
                    <p class="my-0 text-left mt-5"><?= 'Exmo.(s) Sr.(s)' ?></p>
                    <p class="my-0 text-left">
                        <Strong><?= isset($customer) && !empty($customer) ? $customer->name : 'Nome do cliente' ?></Strong>
                    </p>
    ...
    <footer>
                    ...
                    <table class="table table-small">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="border-top-0 text-right"><?= $tax = $video_amount * 0.16 ?></td>
                                <td class="border-top-0 text-right"><?= this_number_format($tax, 2) ?></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                        ...
                        <table class="table table-small table-small-border pb-0 mb-0">
                            <tbody>
                                <tr>
                                    ...
                                    <td class="text-right"><?= this_number_format($subtotal, 2) ?></td>
                                </tr>
                                <tr>
                                    <td class="border-right">Desconto Comercial</td>
                                    <td class="text-right"><?= this_number_format(0, 2) ?></td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td class="text-right"><?= this_number_format($tax, 2) ?></td>
                                </tr>
                                <tr>
                                    <td class="border-right">Total do documento</td>
                                    <td class="text-right"><?= this_number_format($total, 2) ?></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
        ...
        <?php else : ?>
            <p class="mb-2 border-top-separator">
                <strong>Total:</strong>
                <span class="float-right"><strong><?= this_number_format($total, 2) ?></strong></span>
            </p>
    ...
    <main class='' style="margin-top: 	<?= ($table === 'invoice' || $table === 'quotation') ? '-95px' : '-120px' ?> !important; margin-bottom:-60px:">
        <?php if (!isset($is_receipt)) : ?>
            <table id="table" class="table table-small">
                ...
                <tbody>
                    <tr class="border-bottom-0">
                        <td class=" border-top-0 text-center">10 </td>
                        <td class=" border-top-0 text-center">dias</td>
                        <td class="border-top-0"> Duração da campanha</td>
                        <td class=" border-top-0 text-right" rowspan="3" style=" border: 1px solid; border-color: inherit"> <?= number_format($video_amount, 2) . ' MT' ?></td>
                        <td class=" border-top-0 text-right" rowspan="3" style=" border: 1px solid; border-color: inherit"><?= number_format($video_amount, 2) . ' MT' ?></td>
                    </tr>
                    <tr class="border-bottom-0">
                        <td class=" border-top-0 text-center">2 </td>
                        <td class=" border-top-0 text-center">qty</td>
                        <td class="border-top-0"> Número de veículos </td>
                    </tr>
                    <tr class="border-bottom-0">
                        <td class=" border-top-0 text-center">2 </td>
                        <td class=" border-top-0 text-center">minutos</td>
                        <td class="border-top-0"> Duração do spot </td>
                    </tr>
                </tbody>
                ...
                <tbody>
                    <?php foreach ($invoices as $invoice) : ?>
                        <tr>
                            ...
                            <td class="border-to-0 text-right"><?= this_number_format($invoice->invoice_total, 2) ?></td>
                            <td class="border-to-0 text-right"><?= this_number_format($this->invoice_model->get_debt($invoice->invoice_id), 2) ?></td>
                            <td class="border-to-0 text-right"><?= this_number_format($invoice->value, 2) ?></td>
                            <td class="border-to-0 text-right"><?= this_number_format($this->invoice_model->get_debt($invoice->invoice_id), 2) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
